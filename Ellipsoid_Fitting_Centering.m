function q=Ellipsoid_Fitting_Centering(x,nit)
...
%if cond(M)>1e8
%disp('Your point configuration is not correct')
%else
%    ...
%end
%
...
c=-c;
q=proxf2(q);
decenter=[0;0;0;0;0;0;...
    2*q(1)*c(1)+q(4)*c(2)+q(5)*c(3);...
    2*q(2)*c(2)+q(4)*c(1)+q(6)*c(3);...
    2*q(3)*c(3)+q(5)*c(1)+q(6)*c(2);...
    q(1)*c(1)^2+q(2)*c(2)^2+q(3)*c(3)^2+q(4)*c(1)*c(2)+q(5)*c(1)*c(3)+q(6)*c(2)*c(3)+q(7)*c(1)+q(8)*c(2)+q(9)*c(3)];
q=q+decenter;
